<template>
  <md-part>
    ...
    <md-part-body class="no-padding">
      <md-query @select="select" @dblclick="edit" :md-init="initQuery" ref="list" md-query-id="suite.amiba.allot.method.list"></md-query>
...
export default {
  ...
  methods: {
    create() {
      this.$router.push({ name: 'module', params: { module: 'amiba.allot.method.edit' } });
    },
    edit(item) {
      this.$router.push({ name: 'id', params: { module: 'amiba.allot.method.edit', id: item.id } });
    },
    doFetch(q) {
      if (this.currentQ != q) {
        this.currentQ = q;
    ...
    remove() {
      ...
      this.$http.delete('amiba/allot-methods/' + ids).then(response => {
        this.loadData();
        this.loading--;
