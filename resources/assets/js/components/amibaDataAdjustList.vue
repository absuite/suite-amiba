<template>
  <md-part>
    ...
    <md-part-body class="no-padding">
      <md-query @select="select" @dblclick="edit" ref="list" md-query-id="suite.amiba.data.adjust.list"></md-query>
...
export default {
  ...
  methods: {
    create() {
      this.$router.push({ name: 'module', params: { module: 'amiba.data.adjust.edit' } });
    },
    edit(item) {
    ...
    remove() {
      ...
      this.$http.delete('amiba/data-adjusts/' + ids).then(response => {
        this.loadData();
        this.loading--;
