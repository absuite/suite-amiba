<template>
  <md-part>
    ...
    <md-part-body>
      <md-query @select="select" @dblclick="edit" ref="list" md-query-id="suite.amiba.data.doc.list"></md-query>
...
<script>
  export default {
    ...
    methods: {
      create(){
        this.$router.push({ name: 'module', params: { module: 'amiba.data.doc.edit' }});
      },
      edit(item){
        this.$router.push({ name: 'id', params: { module: 'amiba.data.doc.edit',id:item.id }});
      },
      remove(){
        ...
        this.$http.delete('amiba/data-docs/'+ids).then(response => {
          this.load();
          this.loading--;
