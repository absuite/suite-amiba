<template>
  <md-part>
    ...
    <md-part-body>
      <md-query @select="select" @dblclick="edit" ref="list" md-query-id="suite.amiba.data.init.list"></md-query>
...
<script>
  export default {
    ...
    methods: {
      create(){
        this.$router.push({ name: 'module', params: { module: 'amiba.data.init.edit' }});
      },
      edit(item){
        this.$router.push({ name: 'id', params: { module: 'amiba.data.init.edit',id:item.id }});
      },
      remove(){
        ...
        this.$http.delete('amiba/data-inits/'+ids).then(response => {
          this.loadData();
          this.loading--;
