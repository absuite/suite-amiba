<template>
  <md-part>
    ...
    <md-part-body>
      <md-query @select="select" @dblclick="edit" ref="list" md-query-id="suite.amiba.data.target.list"></md-query>
...
<script>
  export default {
    ...
    methods: {
      create(){
        this.$router.push({ name: 'module', params: { module: 'amiba.data.target.edit' }});
      },
      edit(item){
        this.$router.push({ name: 'id', params: { module: 'amiba.data.target.edit',id:item.id }});
      },
      remove(){
        ...
        this.$http.delete('amiba/data-targets/'+ids).then(response => {
          this.loadData();
          this.loading--;
