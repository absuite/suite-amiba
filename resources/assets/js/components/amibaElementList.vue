  ...
  <md-part>
    <md-part-toolbar>
      ...
      <md-part-toolbar-crumbs>
        <md-part-toolbar-crumb>核算要素</md-part-toolbar-crumb>
        <md-part-toolbar-crumb>列表</md-part-toolbar-crumb>
      </md-part-toolbar-crumbs>
    </md-part-toolbar>
    <md-part-body>
      <md-query @init="init_model_query" @select="select" @dblclick="edit" ref="list" md-query-id="suite.amiba.element.list"></md-query>
...
export default {
  ...
  methods: {
    create() {
      this.$router.push({ name: 'module', params: { module: 'amiba.element.edit' } });
    },
    edit(item) {
    ...
    remove() {
      ...
      this.$http.delete('amiba/elements/' + ids).then(response => {
        this.load();
        this.loading--;
