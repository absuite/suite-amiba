<template>
  <md-part>
    ...
    <md-part-body class="no-padding">
      <md-query @select="select" :md-init="initQuery" @dblclick="edit" ref="list" md-query-id="suite.amiba.price.adjust.list"></md-query>
...
export default {
  ...
  methods: {
    create() {
      this.$router.push({ name: 'module', params: { module: 'amiba.price.adjust.edit' } });
    },
    edit(item) {
    ...
    remove() {
      ...
      this.$http.delete('amiba/price-adjusts/' + ids).then(response => {
        this.loadData();
        this.loading--;
