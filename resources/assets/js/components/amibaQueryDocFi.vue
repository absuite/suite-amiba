  ...
  <md-part>
    <md-part-toolbar>
      ...
      <md-part-toolbar-group class="flex">
        <md-layout md-gutter>
          <md-layout md-flex-sm="33" md-flex="15" md-flex-lg="15">
    ...
    <md-part-body class="no-padding">
      <md-query @select="select" :md-init="initQuery" ref="list" md-query-id="suite.amiba.doc.fi.list"></md-query>
...
export default {
  data() {
    return {
      selectRows: [],
      loading: 0,
      model: { period: this.$root.configs.period, biz_type: '', doc_type: '', org: '', account: '' }
    };
  },
  methods: {
    select(items) {
    ...
    initQuery(options) {
      ...
      if (this.model.dept) {
        options.wheres.$dept = { 'like': { 'fm_dept': this.model.dept } };
      }
      if (this.model.account) {
        options.wheres.$account = { 'like': { 'account': this.model.account } };
      }
    },
    loadData() {
